const express = require('express')
const cors = require('cors')
const jwt = require('jsonwebtoken')
const { MongoClient, ServerApiVersion, ObjectId } = require('mongodb');
require('dotenv').config()
const app = express();
const port = process.env.PORT || 5000;


app.use(cors())
app.use(express.json())


app.get('/', (req, res) => {
    res.send('server is running')
})

const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASSWORD}@cluster0.edmdo65.mongodb.net/?retryWrites=true&w=majority`;

// Create a MongoClient with a MongoClientOptions object to set the Stable API version
const client = new MongoClient(uri, {
    serverApi: {
        version: ServerApiVersion.v1,
        strict: true,
        deprecationErrors: true,
    }
});


async function run() {
    try {
        // Connect the client to the server	(optional starting in v4.7)
        const database = client.db('carsDoctor');
        const collection = database.collection('services');
        const bookingCollection = database.collection('bookings');

        app.get('/services', async (req, res) => {
            const data = collection.find();
            const resultedData = await data.toArray();
            res.send(resultedData);
        })


        app.post('/bookings', async (req, res) => {
            const data = req.body;
            const bookings = await bookingCollection.insertOne(data);
            res.send(bookings);
        })

        app.get('/bookings', verifyJWT, async (req, res) => {
            console.log(req.headers.authorization);
            let query = {};
            if (req.query?.email) {
                query = { email: { $eq: req.query.email } }
            }
            const data = bookingCollection.find(query);
            const resultedData = await data.toArray();
            res.send(resultedData);
        })

        app.get('/services/:id', async (req, res) => {
            const id = req.params.id;
            const query = { _id: new ObjectId(id) }
            const options = { projection: { title: 1, price: 1, service_id: 1, img: 1 } }
            const data = await collection.findOne(query, options);
            res.send(data);
        })

        app.delete('/bookings/:id', async (req, res) => {
            const id = req.params.id;
            const query = { _id: new ObjectId(id) }
            const data = await bookingCollection.deleteOne(query)
            res.send(data)
        })

        app.put('/bookings/:id', async (req, res) => {
            const id = req.params.id;
            const sentData = req.body;
            const updatedData = {
                $set: {
                    status: sentData.status,
                }
            }
            const query = { _id: new ObjectId(id) }
            const options = { upsert: true }
            const data = await bookingCollection.updateOne(query, updatedData, options)
            res.send(data)
        })


        // Send a ping to confirm a successful connection
        await client.db("admin").command({ ping: 1 });
        console.log("Pinged your deployment. You successfully connected to MongoDB!");
    } finally {
        // Ensures that the client will close when you finish/error

    }
}
run().catch(console.dir);

app.listen(port, () => console.log(port))
